...
test('v2 identity exposes stormIntelligence primary resource with deprecated weatherRadar alias', async()=>{
  ...
  assert.equal(plugin.version,'2.5.6')
...
test('package exposes current Signal K App Store metadata and published assets',()=>{
  ...
  assert.equal(pkg.signalk.appIcon,'./public/assets/storm-intelligence-icon.png')
...
test('operational route is registered through readonly router and returns component health',async()=>{
  ...
  assert.equal(statusCode,200);assert.equal(payload.readOnly,true);assert.ok(Array.isArray(payload.components));assert.ok(Array.isArray(payload.approachingCells));assert.equal(payload.runtime.version,'2.5.6');assert.match(payload.semantics.risk,/not a probability/i)
  plugin.stop()
})
