...
const { centroidGeometry, pathPolygonThreat } = require('../lib/storm-engine')
...
test('failed radar frame is retried and operational error recovers after success', async () => {
  ...
  const plugin=makePlugin(app)
  try {
    plugin.start({backgroundEnabled:true,pollSeconds:300,enabledProviders:[providerId],displayLayers:[],acquisitionTargets:[target],stormSource:target,inferenceAlgorithms:[algorithmId],lightningEnabled:false,onboardEnvironmentEnabled:false,weatherApiObservationsEnabled:false,prefetchEnabled:false})
