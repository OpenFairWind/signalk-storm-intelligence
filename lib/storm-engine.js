...
function centroidGeometry(g) {
  ...
  for(const polygon of polygons) for(const [ringIndex,ring] of (polygon||[]).entries()){
    ...
    for(let i=0;i<ring.length;i++)if(Array.isArray(ring[i])&&ring[i].length>=2&&(i===0||ring[i][0]!==ring[i-1][0]||ring[i][1]!==ring[i-1][1]))fallback.push(ring[i])
  ...
  if(Math.abs(totalWeight)>1e-12)return[weightedX/totalWeight,weightedY/totalWeight]
  if(!fallback.length)return null
  const unique=fallback.filter((p,i)=>i===0||p[0]!==fallback[0][0]||p[1]!==fallback[0][1]||i<fallback.length-1)
  return [unique.reduce((s,p)=>s+p[0],0)/unique.length,unique.reduce((s,p)=>s+p[1],0)/unique.length]
}
...
function localVectorMeters(origin, target) {
  ...
  const east = rad(target[0]-origin[0])*R*Math.cos(rad((origin[1]+target[1])/2))
  return {east,north}
}
...
function pointInRing(point, ring) {
  ...
  for(let i=0,j=ring.length-1;i<ring.length;j=i++){
    ...
    const hit=((yi>point[1])!==(yj>point[1])) && (point[0] < (xj-xi)*(point[1]-yi)/((yj-yi)||1e-15)+xi)
...
module.exports={StormEngine,haversine,bearing,centroidGeometry,velocityBetween,cpa,severityValue,distanceToGeometryMeters,robustTrackVelocity,pathPolygonThreat,pointInGeometry,translateGeometryLocal}
