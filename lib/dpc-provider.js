...
const { parseHrdZip, parseHrdBinary } = require('./hrd-parser')
const { V2_PRODUCTS, v2TileUrl, decodeV2Tile, transparentPng } = require('./dpc-v2-raster')
...
class DpcProvider {
  ...
  async latest(product){
    if(product==='HRD')return this.latestHrd()
    const u=new URL('/findLastProductByType',this.cfg.dpcApiBase);u.searchParams.set('type',product);u.searchParams.set('lang','it')
  ...
  hrdUrl(epochMs){return new URL(`HRD/hrd_5min_${Number(epochMs)}.bin`,`${this.cfg.dpcHrdBinaryBase.replace(/\/?$/,'/')}`).toString()}
  async latestHrd(){
    const clock=await this.latest('VMI'),periodMs=300000,maxLagFrames=12
    for(let lag=0;lag<=maxLagFrames;lag++){
      const epochMs=clock.epochMs-lag*periodMs
      const response=await fetch(this.hrdUrl(epochMs),{method:'HEAD',headers:{'user-agent':USER_AGENT},signal:timeoutSignal(this.cfg.requestTimeoutMs)})
      if(response.ok)return {product:'HRD',time:new Date(epochMs).toISOString(),epochMs,period:'PT5M',source:this.id}
      if(response.status!==403&&response.status!==404)throw new Error(`Radar-DPC HRD discovery HTTP ${response.status}`)
    }
    throw new Error('No HRD product available within the last 60 minutes')
  }
  ...
  async tile(product,tileRequest,time){
    if(V2_PRODUCTS[product]){
      const request=Array.isArray(tileRequest)?null:tileRequest
      if(!request)throw new Error('Radar-DPC v2 requires an XYZ tile request')
      const frame=time||await this.latest(product)
      const {url,tile}=v2TileUrl(this.cfg.dpcWebpTileBase,product,frame.time||frame,request)
      const r=await fetch(url,{headers:{accept:'image/webp','user-agent':USER_AGENT},signal:timeoutSignal(this.cfg.requestTimeoutMs)})
      if(r.status===403||r.status===404)return transparentPng()
      if(!r.ok)throw new Error(`Radar-DPC v2 tile HTTP ${r.status}`)
      const ct=r.headers.get('content-type')||''
      if(!ct.includes('image/webp'))throw new Error(`Radar-DPC v2 tile returned ${ct}`)
      return decodeV2Tile(product,Buffer.from(await r.arrayBuffer()),tile)
    }
    const bbox=Array.isArray(tileRequest)?tileRequest:tileRequest?.bbox3857
    if(!Array.isArray(bbox))throw new Error('Radar-DPC requires bbox3857 in tile request')
    const r=await fetch(this.wmsUrl(product,bbox,time),{headers:{accept:'image/png,image/*;q=0.8','user-agent':USER_AGENT},signal:timeoutSignal(this.cfg.requestTimeoutMs)})
    if(!r.ok)throw new Error(`Radar-DPC WMS HTTP ${r.status}`)
    const ct=r.headers.get('content-type')||''
    if(!ct.includes('image/png'))throw new Error(`Radar-DPC WMS returned ${ct}`)
    return Buffer.from(await r.arrayBuffer())
  }
  async downloadRaw(product,epochMs){
    if(product==='HRD'){
      const response=await fetch(this.hrdUrl(epochMs),{headers:{'user-agent':USER_AGENT},signal:timeoutSignal(Math.max(this.cfg.requestTimeoutMs,30000))})
      if(!response.ok)throw new Error(`Radar-DPC HRD download HTTP ${response.status}`)
      return {buffer:Buffer.from(await response.arrayBuffer()),key:`hrd_5min_${Number(epochMs)}.bin`,contentType:response.headers.get('content-type')||''}
    }
    const u=new URL('/downloadProduct',this.cfg.dpcApiBase)
  ...
  rawExtension(product,key=''){ if(/\.zip$/i.test(key))return '.zip'; if(product==='HRD'||/\.bin$/i.test(key))return '.bin'; if(/\.tiff?$/i.test(key)||PRODUCTS[product]?.kind==='raster')return '.tif'; return '.bin' }
  async cellsFromRaw(product,buffer){ if(product!=='HRD')return []; return buffer?.[0]===72?parseHrdBinary(buffer):parseHrdZip(buffer) }
}
